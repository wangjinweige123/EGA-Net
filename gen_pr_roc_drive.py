...
def collect_pairs(drive_dir: Path):
...
def build_and_load_model(model_name: str, weight_path: Path, device: torch.device):
    ...
    if model_name.lower() in ["unet", "unet_saspp_ega"]:
        use_saspp = "saspp" in model_name.lower()
        use_ega = "ega" in model_name.lower()
...
def plot_with_insets(results, out_dir: Path, auc_map: dict):
    ...
    ax2_inset.set_xlim([0.04, 0.1])  
    ax2_inset.set_ylim([0.88, 0.97])
...
def main():
    parser = argparse.ArgumentParser(description="Generate PR/ROC curves on DRIVE test set.")
    parser.add_argument("--drive_dir", type=str, required=True, 
                        help="Root of DRIVE (contains test/image and test/label).")
    ...
    parser.add_argument("--eval_sizes", type=str, 
                        default="transunet:512,medsegdiff:384,unet:576,unetplusplus:576,mobilenetv3_lraspp:576,unet_saspp_ega:576")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--output_dir", type=str, default=None, 
                        help="Output dir for figures (default <DRIVE>/test/visualization).")
    parser.add_argument("--results_csv", type=str, default=None, 
                        help="results_unified_drive.csv path to pull AUC numbers into legends.")
    ...
    out_dir = Path(args.output_dir) if args.output_dir else (drive_dir / "test" / "visualization")
    device = torch.device(args.device)

    default_weights = {
        "mobilenetv3_lraspp": str(drive_dir / "checkpoint_unified_models/mobilenetv3_lraspp/mobilenetv3_lraspp_best.pth"),
        "unet":                str(drive_dir / "checkpoint_unified_models/unet/UNet_best.pth"),
        "medsegdiff":          str(drive_dir / "checkpoint_unified_models/medsegdiff/medsegdiff_best.pth"),
        "transunet":           str(drive_dir / "checkpoint_unified_models/transunet/transunet_best.pth"),
        "unetplusplus":        str(drive_dir / "checkpoint_unified_models/unetplusplus/unetplusplus_best.pth"),
        "unet_saspp_ega":      str(drive_dir / "test/checkpoint/UNet_drive_saspp_ega.pth"),
    }
    ...
    results_csv = Path(args.results_csv) if args.results_csv else (drive_dir / "results_unified_drive.csv")
    auc_map = load_auc_from_csv(results_csv)

    model_names = [m.strip() for m in args.models.split(",") if m.strip()]
    size_map = parse_eval_sizes(args.eval_sizes)
    pairs = collect_pairs(drive_dir)

    print(f"\n📊 Found {len(pairs)} test pairs in DRIVE")
    ...
    for name in model_names:
        ...
        if not wp or not Path(wp).exists():
            ...
            continue
        eval_size = size_map.get(name, 576)
        try:
