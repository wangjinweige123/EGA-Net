...
parser = argparse.ArgumentParser(description='UNet Training with/without SASPP and EGA - Drive Dataset')
...
def load_data(files, img_dir, lbl_dir):
    ...
    for fn in files:
        ...
        lbl_path = os.path.join(lbl_dir, f"{fn.split('.')[0]}.png")

        im_exists = os.path.exists(im_path)
        lbl_exists = os.path.exists(lbl_path)
        
        if not im_exists or not lbl_exists:
            print(f"文件不存在: {fn}")
            print(f"  图像存在: {im_exists} - {im_path}")
            print(f"  标签存在: {lbl_exists} - {lbl_path}")
        ...
        if im_size == 0 or lbl_size == 0:
            print(f"文件为空: {fn}")
            print(f"  图像大小: {im_size} bytes")
            print(f"  标签大小: {lbl_size} bytes")
            failed_files.append(fn)
            continue

        im = cv2.imread(im_path, cv2.IMREAD_GRAYSCALE)
        if im is None:
            print(f"CV2无法读取图像: {fn}")
            print(f"  图像路径: {im_path}")
            print(f"  文件大小: {im_size} bytes")

            try:
                from PIL import Image
                pil_img = Image.open(im_path).convert('L')
                im = np.array(pil_img)
                print(f"  PIL成功读取图像: 形状 {im.shape}")
        ...
        if lab is None:
            print(f"CV2无法读取标签: {fn}")
            print(f"  标签路径: {lbl_path}")
            print(f"  文件大小: {lbl_size} bytes")

            try:
                from PIL import Image
                pil_lab = Image.open(lbl_path).convert('L')
                lab = np.array(pil_lab)
                print(f"  PIL成功读取标签: 形状 {lab.shape}")
            except Exception as e:
...
ckpt_dir = 'Drive/test/checkpoint'
loss_dir = 'Drive/test/loss'
...
def check_problematic_files():
    ...
    print("=" * 60)
    print("检查Drive数据集中有问题的文件")
    print("=" * 60)
    
    problematic_files = ['01.png', '02.png', '03.png', '04.png', '05.png']
    
    for fn in problematic_files:
        print(f"\n检查文件: {fn}")
        im_path = os.path.join(training_images_loc, fn)
        lbl_path = os.path.join(training_label_loc, fn)
        ...
        im_exists = os.path.exists(im_path)
        lbl_exists = os.path.exists(lbl_path)
        print(f"图像存在: {im_exists}")
        print(f"标签存在: {lbl_exists}")
        ...
        if im_exists:
            ...
            try:
                im_cv2 = cv2.imread(im_path, cv2.IMREAD_GRAYSCALE)
                if im_cv2 is not None:
                    print(f"CV2读取成功: 形状 {im_cv2.shape}")
                else:
                    print("CV2读取失败")
            except Exception as e:
                print(f"CV2读取异常: {e}")
            ...
            try:
                ...
                print(f"PIL读取成功: 模式 {im_pil.mode}, 尺寸 {im_pil.size}")
            except Exception as e:
                print(f"PIL读取异常: {e}")
        ...
        if lbl_exists:
            ...
            try:
                lbl_cv2 = cv2.imread(lbl_path, cv2.IMREAD_GRAYSCALE)
                if lbl_cv2 is not None:
                    print(f"CV2读取标签成功: 形状 {lbl_cv2.shape}")
                else:
                    print("CV2读取标签失败")
            except Exception as e:
                print(f"CV2读取标签异常: {e}")
            ...
            try:
                ...
                print(f"PIL读取标签成功: 模式 {lbl_pil.mode}, 尺寸 {lbl_pil.size}")
            except Exception as e:
                print(f"PIL读取标签异常: {e}")

def verify_paths_and_files():
    """验证路径和文件状态"""
    print("=" * 60)
    print("验证路径和文件状态")
    print("=" * 60)
    ...
    directories = [
        ('训练图像目录', training_images_loc),
        ('训练标签目录', training_label_loc), 
        ('验证图像目录', validate_images_loc),
        ('验证标签目录', validate_label_loc),
        ('测试图像目录', 'Drive/test/image/'),
        ('测试标签目录', 'Drive/test/label/')
    ]
    ...
    print("检查01-05.png在不同目录中的状态:")
    print("-" * 40)
    
    for fn in problematic_files:
        print(f"\n文件: {fn}")

        train_img = os.path.join(training_images_loc, fn)
        train_lbl = os.path.join(training_label_loc, fn)
        print(f"  训练图像: {os.path.exists(train_img)} - {train_img}")
        print(f"  训练标签: {os.path.exists(train_lbl)} - {train_lbl}")
 
        test_img = os.path.join('Drive/test/image/', fn)
        test_lbl = os.path.join('Drive/test/label/', fn)
        print(f"  测试图像: {os.path.exists(test_img)} - {test_img}")
        print(f"  测试标签: {os.path.exists(test_lbl)} - {test_lbl}")

        if os.path.exists(train_img) and os.path.exists(test_img):
            train_size = os.path.getsize(train_img)
            test_size = os.path.getsize(test_img)
            print(f"  训练图像大小: {train_size} bytes")
            print(f"  测试图像大小: {test_size} bytes")
            print(f"  大小相同: {train_size == test_size}")
...
def main():
    ...
    train_loader = DataLoader(DriveDataset(x_train, y_train), batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(DriveDataset(x_val, y_val), batch_size=batch_size, shuffle=False)
    ...
    writer = SummaryWriter(log_dir=f'logs/drive_{module_name}')

    weight_path = os.path.join(ckpt_dir, f'UNet_drive_{module_name}.pth')
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'Drive Dataset - Loss Curve ({module_name.upper()})')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    loss_curve_path = os.path.join(loss_dir, f'loss_drive_{module_name}.png')
