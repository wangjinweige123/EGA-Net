...
def _find_label_for(img_path: Path, label_dir: Path):
    stem = img_path.stem
    cands = []
    cands += sorted(label_dir.glob(f"{stem}_1stHO.*"))
    cands += sorted(label_dir.glob(f"{stem}.*"))
    cands += sorted(label_dir.glob(f"{stem}_*.*"))
...
def collect_pairs(chase_dir: Path):
    test_dir = chase_dir / "test"
    img_dir = test_dir / "image"
    label_dir = (test_dir / "label") if (test_dir / "label").exists() else (test_dir / "labe")
...
def build_and_load_model(model_name: str, weight_path: Path, device: torch.device):
    ...
    total = len(msd)
    print(f"[load] {model_name}: matched {best_match}/{total} ({100.0*best_match/max(1,total):.1f}%)")
...
def infer_prob(model_name: str, model: nn.Module, image_np: np.ndarray, device: torch.device) -> np.ndarray:
    ...
    y = forward_model(model_name, model, x)   # handles medsegdiff (t=0)
...
def load_auc_from_csv(csv_path: Path):
    """
    Robust reader: model name column can be 'Model'/'model'/'name'.
    PR-AUC column matched by substring 'auc_pr' or 'ap'.
    ROC-AUC column matched by 'auc' but not 'pr'.
    """
...
def label_from_csv_or_calc(name: str, pr_calc: float, roc_calc: float, auc_map: dict):
    ...
    # inject your UNet_saspp_ega ROC if CSV lacks it
...
def plot_with_insets(results, out_dir: Path, auc_map: dict):
    ...
    ax1_inset.set_xlim([0.70, 0.80])
    ax1_inset.set_ylim([0.72, 0.88])
    ...
    ax2_inset.set_xlim([0.02, 0.07]) 
    ax2_inset.set_ylim([0.89, 0.97])
...
def main():
    parser = argparse.ArgumentParser(description="Generate PR/ROC curves on CHASE test set.")
    parser.add_argument("--chase_dir", type=str, required=True, help="Root of CHASE (contains test/image and test/label or test/labe).")
    parser.add_argument("--models", type=str, default="mobilenetv3_lraspp,unet,medsegdiff,transunet,unetplusplus,unet_saspp_ega")
    parser.add_argument("--eval_sizes", type=str, default="transunet:512,medsegdiff:384,unet:1008,unetplusplus:1008,mobilenetv3_lraspp:1008,unet_saspp_ega:1008")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--output_dir", type=str, default=None, help="Output dir for figures (default <CHASE>/test/visualization).")
    parser.add_argument("--results_csv", type=str, default=None, help="results_unified_chase.csv path to pull AUC numbers into legends.")
    parser.add_argument("--weights_json", type=str, default=None, help="Optional JSON {model: weight_path}")
    parser.add_argument("--weight", action="append", default=None, help="Override pairs, format name=path (repeatable).")
    args = parser.parse_args()

    chase_dir = Path(args.chase_dir)
    out_dir = Path(args.output_dir) if args.output_dir else (chase_dir / "test" / "visualization")
    device = torch.device(args.device)

    # default relative weights
    default_weights = {
        "mobilenetv3_lraspp": str(chase_dir / "checkpoint_unified_models/mobilenetv3_lraspp/mobilenetv3_lraspp_best.pth"),
        "unet":                str(chase_dir / "checkpoint_unified_models/unet/UNet_best.pth"),
        "medsegdiff":          str(chase_dir / "checkpoint_unified_models/medsegdiff/medsegdiff_best.pth"),
        "transunet":           str(chase_dir / "checkpoint_unified_models/transunet/transunet_best.pth"),
        "unetplusplus":        str(chase_dir / "checkpoint_unified_models/unetplusplus/unetplusplus_best.pth"),
        "unet_saspp_ega":      str(chase_dir / "test/checkpoint/UNet_saspp_ega.pth"),
    }

    # weight map with overrides
    ...
    # results csv (for legends)
    results_csv = Path(args.results_csv) if args.results_csv else (chase_dir / "test" / "results_unified_chase.csv")
    auc_map = load_auc_from_csv(results_csv)

    model_names = [m.strip() for m in args.models.split(",") if m.strip()]
    size_map = parse_eval_sizes(args.eval_sizes)
    pairs = collect_pairs(chase_dir)
    ...
    for name in model_names:
        ...
        if not wp or not Path(wp).exists():
            ...
            continue
        eval_size = size_map.get(name, 1008)
        try:
