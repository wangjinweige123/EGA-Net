...
parser = argparse.ArgumentParser(description='评估UNet模型 - Drive数据集')
parser.add_argument('--use_saspp', action='store_true', help='启用SASPP模块')
parser.add_argument('--use_ega', action='store_true', help='启用EGA模块')
parser.add_argument('--weight_path', type=str, default="Drive/test/checkpoint/UNet_drive_base.pth", help='模型权重路径')
...
CONFIG = {
    ...
    'paths': {
        'data_location': 'Drive/',
        'result_dir': './Drive/test/result/result-eval',
        'visual_dir': './Drive/test/visualization/visualization-eval',
        'eval_result_file': './Drive/test/evaluation_results_eval.txt'
    }
}
...
CONFIG['paths']['result_dir'] = f"./Drive/test/result/result-eval{model_suffix}"
CONFIG['paths']['visual_dir'] = f"./Drive/test/visualization/visualization-eval{model_suffix}"
CONFIG['paths']['eval_result_file'] = f"./Drive/test/evaluation_results_eval{model_suffix}.txt"
...
CONFIG['paths']['threshold_result_file'] = f"./Drive/test/threshold_evaluation_results{model_suffix}.txt"
CONFIG['paths']['threshold_csv_file'] = f"./Drive/test/threshold_detailed_results{model_suffix}.csv"
CONFIG['paths']['threshold_plot_file'] = f"./Drive/test/threshold_curves{model_suffix}.png"
...
def debug_file_structure():
    ...
    base_path = CONFIG['paths']['data_location']
    print(f"基础数据路径: {base_path}")
    print(f"路径是否存在: {os.path.exists(base_path)}")

    img_dir = CONFIG['paths']['testing_images_loc']
    print(f"\n图像目录: {img_dir}")
    print(f"目录是否存在: {os.path.exists(img_dir)}")
    
    if os.path.exists(img_dir):
        img_files = os.listdir(img_dir)
        print(f"图像目录中的文件数量: {len(img_files)}")
        print(f"前5个文件: {img_files[:5]}")

        for i, file_name in enumerate(img_files[:3]):
            ...
            try:
                ...
                if img is not None:
                    print(f"    读取成功: 形状 {img.shape}")
                else:
                    print(f"    读取失败: cv2.imread返回None")
            except Exception as e:
                print(f"    读取异常: {e}")

    label_dir = CONFIG['paths']['testing_label_loc']
    print(f"\n标签目录: {label_dir}")
    print(f"目录是否存在: {os.path.exists(label_dir)}")
    
    if os.path.exists(label_dir):
        label_files = os.listdir(label_dir)
        print(f"标签目录中的文件数量: {len(label_files)}")
        print(f"前5个文件: {label_files[:5]}")

        for i, file_name in enumerate(label_files[:3]):
...
def load_test_data():
    ...
    all_img_files = os.listdir(CONFIG['paths']['testing_images_loc'])
    test_files = [f for f in all_img_files if f.lower().endswith('.png')]
    
    print(f"PNG图像文件: {test_files}")
    
    if len(test_files) == 0:
        print(f"目录中的所有文件: {all_img_files}")
        raise ValueError(f"测试目录中没有找到PNG文件: {CONFIG['paths']['testing_images_loc']}")

    if not os.path.exists(CONFIG['paths']['testing_label_loc']):
        raise FileNotFoundError(f"测试标签目录不存在: {CONFIG['paths']['testing_label_loc']}")
    
    all_label_files = os.listdir(CONFIG['paths']['testing_label_loc'])
    print(f"标签目录中的所有文件: {all_label_files}")
    ...
    for i, file_name in enumerate(test_files):
        ...
        if im is None:
            print(f"  错误: cv2.imread无法读取图像文件")

            try:
                import PIL.Image
                pil_img = PIL.Image.open(im_path).convert('L')
                im = np.array(pil_img)
                print(f"  使用PIL成功读取: 形状 {im.shape}")
            except Exception as e:
                print(f"  PIL也无法读取: {e}")
                continue
        else:
            print(f"  成功读取图像: 形状 {im.shape}")

        base_name = file_name.split('.')[0]
        print(f"  查找标签文件，基础名称: {base_name}")

        possible_label_names = [
            f"{base_name}.png",           # 01.png
            f"{base_name}_manual1.png",   # 01_manual1.png  
            f"{base_name}_manual1.gif",   # 01_manual1.gif (常见格式)
            f"{base_name}_test.png",      # 01_test.png
            f"{file_name}",               # 完全相同
        ]
        
        label_path = None
        for label_name in possible_label_names:
            temp_path = os.path.join(CONFIG['paths']['testing_label_loc'], label_name)
            print(f"    尝试: {label_name} -> 存在: {os.path.exists(temp_path)}")
            if os.path.exists(temp_path):
                label_path = temp_path
                break
        
        if label_path is None:
            print(f"  警告: 找不到对应的标签文件")
            print(f"  标签目录中的文件: {[f for f in all_label_files if base_name in f]}")
            continue
        
        print(f"  读取标签: {label_path}")
        ...
        if label is None:
            ...
            try:
                import PIL.Image
                pil_label = PIL.Image.open(label_path).convert('L')
                label = np.array(pil_label)
                print(f"  使用PIL成功读取标签: 形状 {label.shape}")
            except Exception as e:
                print(f"  PIL也无法读取标签: {e}")
                continue
        else:
            print(f"  成功读取标签: 形状 {label.shape}")
        
        file_indices[successful_count] = file_name
        old_size = im.shape[:2]
    ...
    if len(test_data) == 0:
        raise ValueError("没有成功加载任何测试数据，请检查文件路径、文件格式和文件名")
...
def load_model():
    ...
    if os.path.isfile(CONFIG['weight_path']):
        ...
        alt_paths = [
            "Drive/test/checkpoint/UNet_drive_base.pth",
            "Drive/test/checkpoint/UNet_drive_saspp.pth",
            "Drive/test/checkpoint/UNet_drive_ega.pth",
            "Drive/test/checkpoint/UNet_drive_saspp_ega.pth"
        ]
...
def plot_threshold_curves(df_results):
    ...
    plt.title('Drive Dataset: Performance Metrics vs Threshold', fontsize=14)
...
def process_results(test_data, y_test, y_pred, file_indices):
    ...
    new_shape = (len(y_test), 584, 565, 1)  # 注意：Drive是565×584，所以height=584, width=565
...
def save_evaluation_results(metrics):
    ...
    with open(CONFIG['paths']['eval_result_file'], 'w') as f:
        ...
        if 'flops_g' in CONFIG:
            f.write(f'FLOPs (G) @576: {CONFIG["flops_g"]:.2f}\n')
        if 'flops_g_512' in CONFIG:
            f.write(f'FLOPs_512 (G, 归一化): {CONFIG["flops_g_512"]:.2f}\n')
        if 'fps' in CONFIG:
...
def main():
    ...
    flops_side = 576
    total_flops_576 = calculate_flops(model, input_shape=(1, in_ch, flops_side, flops_side))
    params_m = total_params / 1e6
    flops_g  = total_flops_576 / 1e9
    flops_g_512 = flops_g * (512 * 512) / (flops_side * flops_side)

    CONFIG['flops_g_512'] = flops_g_512
