...
def list_drive_test_paths(base_dir):
    base = Path(base_dir)
    paths = {
        "test_img":  base / "Drive" / "test" / "image",
        "test_lbl":  base / "Drive" / "test" / "label",
        "orig_img":  base / "Drive" / "test" / "im",  
    }
    
    print("\n" + "="*70)
    print("检查Drive数据库路径和文件")
    print("="*70)
...
def measure_fps(model_name, model, device, size=576, warmup=20, runs=200):
    model.eval()
...
def estimate_flops(model, device, size=576):
    import torch.nn as nn
    def is_supported(m):
...
def get_original_image(current_file, default_img, orig_images_loc):
    base_name = current_file.replace('.png', '').replace('.jpg', '').replace('.tif', '')
    orig_img_path = os.path.join(orig_images_loc, f"{base_name}.png")
...
def main():
    parser = argparse.ArgumentParser(description="统一评估 - Drive数据库")
    ...
    parser.add_argument("--unet_weight_path", type=str, default="Drive/checkpoint/UNet_best.pth")
    ...
    parser.add_argument("--save_csv", type=str, default="results_unified_drive.csv")
    ...
    paths = list_drive_test_paths(args.data_dir)
    ...
    for model_name in target:
        print(f"\n===== 评估模型: {model_name.upper()} (Drive数据库) =====")
        ...
        if model_name == "unet":
            ...
            candidates.extend([
                wd / "Drive" / "checkpoint" / "UNet_best.pth",
                wd / "Drive" / "checkpoint" / "UNet_base.pth",
                wd / "Drive" / "test" / "checkpoint" / "UNet_best.pth",
                wd / "Drive" / "test" / "checkpoint" / "UNet_base.pth",
                wd / "Drive" / "checkpoint_unified_models" / "unet" / "unet_best.pth",
                wd / "Drive" / "checkpoint_unified_models" / "unet" / "unet_last.pth",
            ])
            ckpt = None
            for p in candidates:
            ...
            if explicit:
                ...
                base_unified = Path(args.weights_dir) / "Drive" / "checkpoint_unified_models" / model_name
