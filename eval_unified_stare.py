...
def list_stare_test_paths(base_dir):
    base = Path(base_dir)
    paths = {
        "test_img":  base / "Stare" / "test" / "image",
        "test_lbl":  base / "Stare" / "test" / "label",
        "orig_img":  base / "Stare" / "test" / "im",  
    }
    
    print("\n" + "="*70)
    print("检查Stare数据库路径和文件")
    print("="*70)
    
    for k, p in paths.items():
        if not p.exists() and k != "orig_img":  
...
def read_pair_full(img_path: Path, lbl_dir: Path, desired):
    ...
    def safe_imread(filepath):
        try:
            ...
            img_pil = Image.open(str(filepath)).convert('L')
    ...
    lab = safe_imread(lbl_path)
    if lab is None:
        file_size = lbl_path.stat().st_size if lbl_path.exists() else 0
        raise RuntimeError(
            f"无法读取标签文件!\n"
            f"  路径: {lbl_path}\n"
            f"  文件存在: {lbl_path.exists()}\n"
            f"  文件大小: {file_size} bytes\n"
            f"  尝试了3种读取方法都失败\n"
            f"  建议: 检查文件是否损坏或格式不正确"
        )
...
def measure_fps(model_name, model, device, size=704, warmup=20, runs=200):
    model.eval()
...
def estimate_flops(model, device, size=704):
    import torch.nn as nn
    def is_supported(m):
...
def get_original_image(current_file, default_img, orig_images_loc):
    base_name = current_file.replace('.ppm', '').replace('.png', '').replace('.jpg', '')
    orig_img_path = os.path.join(orig_images_loc, f"{base_name}.ppm")
    
    if not os.path.exists(orig_img_path):
        for ext in ['.jpg', '.JPG', '.png', '.PNG', '.ppm', '.PPM']:
            alt_path = os.path.join(orig_images_loc, f"{base_name}{ext}")
...
def save_visualizations(model_name, img_files, images, y_true_all, y_prob_all, 
    ...
    for i, (img_file, gray_img, gt_mask, pred_prob) in enumerate(zip(img_files, images, y_true_all, y_prob_all)):
        ...
        orig_img = get_original_image(current_file, gray_img_uint8, orig_images_loc)
        res_dir = Path(paths["test_img"]).parent / "result" / f"result-eval_{model_name}"
        res_dir.mkdir(parents=True, exist_ok=True)

        stem = Path(current_file).stem
        prob_u8 = (np.clip(pred_prob, 0, 1) * 255).astype(np.uint8)
        cv2.imwrite(str(res_dir / f"{i}_{stem}.png"), prob_u8)

        bin_u8  = (pred_mask.astype(np.uint8) * 255)
        cv2.imwrite(str(res_dir / f"{i}_{stem}_bin.png"), bin_u8)
        ...
        overlay_img = create_overlay_image(gray_img_uint8, gt_mask, pred_mask)
        
        canvas = combine_visualizations(orig_img, gt_img, pred_img, overlay_img)
        
        vis_output_path = vis_dir / f"{i}_{current_file.split('.')[0]}_comparison.png"
        cv2.imwrite(str(vis_output_path), canvas)
        print(f"已保存可视化结果: {vis_output_path}")


def main():
    parser = argparse.ArgumentParser(description="统一评估 - Stare数据库")
    ...
    parser.add_argument("--unet_weight_path", type=str, default="Stare/test/checkpoint/UNet_stare_base.pth")
    ...
    parser.add_argument("--save_csv", type=str, default="results_unified_stare.csv")
    ...
    paths = list_stare_test_paths(args.data_dir)
    ...
    img_files = sorted([f for f in Path(paths["test_img"]).iterdir()
                        if f.suffix.lower() in [".png",".jpg",".bmp",".tif",".tiff",".ppm"]])

    rows = []
    for model_name in target:
        print(f"\n===== 评估模型: {model_name.upper()} (Stare数据库) =====")
        ...
        if model_name == "unet":
            ...
            candidates = []
            if args.unet_weight_path:
                candidates.append(Path(args.unet_weight_path))
            
            wd = Path(args.weights_dir)
            candidates.extend([
                wd / "Stare" / "test" / "checkpoint" / "UNet_stare_base.pth",
                wd / "Stare" / "test" / "checkpoint" / "UNet_best.pth",
                wd / "Stare" / "checkpoint" / "UNet_best.pth",
                wd / "Stare" / "checkpoint" / "UNet_base.pth",
                wd / "Stare" / "checkpoint_unified_models" / "unet" / "unet_best.pth",
                wd / "Stare" / "checkpoint_unified_models" / "unet" / "unet_last.pth",
            ])
            ...
            if ckpt is None:
                ...
                raise FileNotFoundError(
                    f"未找到 UNet 权重。已尝试路径:{tried}\n"
                    f"可通过 --unet_weight_path 指定"
                )
        else:
            explicit = {
            ...
            if explicit:
                ...
                base_unified = Path(args.weights_dir) / "Stare" / "checkpoint_unified_models" / model_name
